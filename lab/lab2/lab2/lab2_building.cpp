...
struct Building {
	...
	void initialize(glm::vec3 position, glm::vec3 scale) {
		...
		textureID = textures[randomNumber];
...
int main(void)
{
	...
	int gridWidth = 5; // Nombre de colonnes
	int gridHeight = 5; // Nombre de lignes
	float spacing = 100.0f; // Espacement entre les bâtiments

	for (int i = -gridHeight/2; i < gridHeight/2; i++) {
		for (int j = -gridWidth/2; j < gridWidth/2; j++) {

			std::vector<int> values = {16, 32, 48};
			std::random_device rd;
			std::default_random_engine generator(rd());

			// Distribution pour le premier vecteur
			std::uniform_int_distribution<int> distribution1(0, values.size() - 1);
			int randomIndex1 = distribution1(generator);
			int randomValue1 = values[randomIndex1]; // Valeur aléatoire du premier vecteur

			std::vector<int> values2 = {1, 2, 3};

			// Distribution pour le second vecteur
			std::uniform_int_distribution<int> distribution2(0, values2.size() - 1);
			int randomIndex2 = distribution2(generator);
			int randomValue2 = values2[randomIndex2];

			std::cout << "Random number " << randomValue1 << std::endl;
			Building building;
			glm::vec3 position(j * spacing, -50.0f + randomValue1, i * spacing); // Placer les bâtiments sur le plan XZ
			glm::vec3 scale(16.0f * randomValue2, randomValue1, 16.0f * randomValue2);
			building.initialize(position, scale);
