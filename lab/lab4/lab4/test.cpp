...
struct MyBot {
	...
	{
		// Récupérer le nœud correspondant
		const tinygltf::Node& node = model.nodes[nodeIndex];

		glm::mat4 transform(1.0f);

		// Si une matrice complète est définie, elle a priorité
		if (node.matrix.size() == 16) {
			transform = glm::make_mat4(node.matrix.data());
		} else {
		...
		// Stocker la transformation locale calculée dans le vecteur
	...
	void updateSkinning(const std::vector<glm::mat4> &nodeTransforms) {
		...
		for (size_t i = 0; i < skinObjects.size(); ++i) {
			...
			// Mettre à jour les joint matrices en fonction des transformations des nœuds
			for (size_t j = 0; j < skinObject.jointMatrices.size(); ++j) {
				int nodeIndex = model.skins[i].joints[j]; // Index du nœud correspondant au joint
	...
	void update(float time) {
		if (model.animations.size() > 0) {
			...
			// Récupérer la première peau (skin) du modèle
			const tinygltf::Skin &skin = model.skins[0];

			// Initialiser les transformations locales pour chaque joint
			std::vector<glm::mat4> nodeTransforms(model.nodes.size(), glm::mat4(1.0f));

			// Calculer les transformations locales à partir des animations
			updateAnimation(model, animation, animationObject, time, nodeTransforms);

			// Calculer les transformations globales
			std::vector<glm::mat4> globalNodeTransforms(model.nodes.size(), glm::mat4(1.0f));
			int rootNodeIndex = skin.skeleton; // Obtenir le nœud racine du squelette
			computeGlobalNodeTransform(model, nodeTransforms, rootNodeIndex, glm::mat4(1.0f), globalNodeTransforms);

			// Appeler `updateSkinning` pour gérer le skinning avec les transformations globales
