...
struct MyBot {
	...
	void update(float time) {
		...
		if (model.animations.size() > 0) {
			...
			// Récursivement propager les transformations globales
			...
			// Mettre à jour les transformations globales
	...
	void initialize() {
		...
		for (int jointIndex : skin.joints) {
			computeLocalNodeTransform(model, jointIndex, localNodeTransforms);
		}
